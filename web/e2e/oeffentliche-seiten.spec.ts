...
test.describe("Startseite", () => {
  ...
  test("über HTTPS erzwingt die Seite HTTPS auch für die Zukunft (HSTS)", async ({
    page,
    baseURL,
  }) => {
    /*
    ...
    */
    test.skip(
      ...
      "Nur gegen eine HTTPS-Umgebung aussagekräftig.",
    );
...
test.describe("Preise", () => {
  test("zeigt alle drei Tarife mit Monatspreisen", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Kostenlos" }),
    ).toBeVisible();
...
test.describe("Rechtstexte", () => {
  ...
  for (const [pfad, ueberschrift] of [
    ...
    test(`${pfad} ist erreichbar`, async ({ page }) => {
      ...
      await expect(
        page
          .getByRole("heading", { name: new RegExp(ueberschrift, "i") })
          .first(),
      ).toBeVisible();
    });
  }
