...
- [x] **Checkout + Webhook implementiert** (Sandbox/Testmodus, `acct_1TvfGDQiFozdcJ4r`). Vier Preise angelegt (Basic 9,99€/Monat · 99,90€/Jahr, Pro 19,99€/Monat · 199,90€/Jahr, IDs in `.env.local`). Neue Migration [`20260729100000_stripe_subscriptions.sql`](web/supabase/migrations/20260729100000_stripe_subscriptions.sql): `upsert_subscription()` spiegelt Plan/Status bei jedem `customer.subscription.*`-Event, `grant_subscription_credits()` schreibt Credits **ausschließlich** bei `invoice.paid` gut (idempotent über den bereits vorhandenen Unique-Index auf `credit_ledger.stripe_event_id`) — bewusst getrennt, damit ein Statuswechsel (z. B. `cancel_at_period_end` umschalten) nicht versehentlich Credits auslöst. Jahresabos bekommen die 12-fache Monatsmenge auf einmal gutgeschrieben (ökonomisch unproblematisch, da Credits nicht verfallen). `/preise` hat jetzt einen echten Monats-/Jahres-Umschalter mit funktionierendem Checkout-Button, `/konto` einen Link ins Stripe Customer Portal (Kündigung/Zahlungsmittel). `tsc`, `npm run build`, Preise-Seite und Footer-Text live geprüft (unangemeldeter Checkout-Versuch liefert korrekt 401 → Weiterleitung zu `/anmelden`).
- [ ] **`STRIPE_WEBHOOK_SECRET` in `.env.local` ist noch leer.** Lokal per `stripe listen --forward-to localhost:3000/api/stripe/webhook` erzeugen (Stripe CLI ist auf dieser Maschine noch nicht installiert) — ohne diesen Wert antwortet der Webhook-Endpoint mit 500.
- [ ] **Vollständiger End-to-End-Test mit echtem Login steht noch aus** — bisher nur der unangemeldete Pfad (401) live geprüft, da kein Test-Account mit bekanntem Passwort zur Hand war. Als angemeldeter Nutzer einmal durchklicken: „Jetzt upgraden" → Stripe-Checkout-Seite mit korrektem Betrag → Testkarte `4242 4242 4242 4242` → zurück auf `/konto` → Plan/Credits aktualisiert. Zusätzlich `4000 0000 0000 0341` (erzwungener Fehlschlag) prüfen.
- [ ] **Für den Produktivbetrieb**: Webhook-Endpoint im Stripe-Dashboard unter der echten Domain registrieren (`checkout.session.completed` wird aktuell absichtlich nicht abonniert — siehe Kommentar im Webhook-Handler; benötigte Events: `customer.subscription.created/updated/deleted`, `invoice.paid`, `invoice.payment_failed`) und den daraus resultierenden `whsec_...` in die Produktions-Umgebungsvariablen eintragen.
